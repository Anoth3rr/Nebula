...
public static partial class AppConfig
{
    ...
    public static string? NebulaPortableLauncherExecutePath { get; private set; }
    ...
    public static async Task CheckEnviromentAsync()
    {
        try
        {
            ...
            string portableExe = Path.Join(parentFolder, "Nebula.exe");
            ...
            if (Directory.Exists(parentFolder) && (File.Exists(portableExe) || File.Exists(portableVersion)))
            {
                ...
                NebulaPortableLauncherExecutePath = portableExe;
            ...
            {
                CacheFolder = Path.Combine(parentFolder!, ".NebulaCache");
                ConfigPath = Path.Combine(parentFolder!, "config.ini");
            }
            else if (IsAppInRemovableStorage)
            {
                CacheFolder = Path.Combine(Path.GetPathRoot(AppContext.BaseDirectory)!, ".NebulaCache");
                ConfigPath = Path.Combine(CacheFolder, "config.ini");
            }
            else if (IsPortable)
            {
                CacheFolder = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "Nebula");
                ConfigPath = Path.Combine(parentFolder!, "config.ini");
            }
            else
            {
            ...
            {
#if DEBUG
                using RegistryKey key = Registry.CurrentUser.CreateSubKey(@"Software\Nebula.Debug");
#else
                using RegistryKey key = Registry.CurrentUser.CreateSubKey(@"Software\Nebula");
#endif
    ...
    {
#if DEBUG
        using var key = Registry.CurrentUser.CreateSubKey(@"Software\Nebula.Debug");
#else
        using var key = Registry.CurrentUser.CreateSubKey(@"Software\Nebula");
#endif
