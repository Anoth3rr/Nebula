...
namespace Nebula.Features.GameSetting;
...
public sealed partial class GameSettingPage : PageBase
{
    ...
    private async Task InitializeGameSettingAsync()
    {
        try
        {
            bool isThirdPartyLocalGame = CurrentGameBiz.Game is GameBiz.arknights or GameBiz.endfield or GameBiz.wutheringwaves;
            var localVersion = isThirdPartyLocalGame ? null : await _gameLauncherService.GetLocalGameVersionAsync(CurrentGameId);
            if (!isThirdPartyLocalGame && localVersion is null)
            {
