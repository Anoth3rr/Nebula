...
public sealed partial class GameSelector : UserControl
{
    ...
    public void UpdateDragRectangles()
    {
        try
        {
            ...
            {
                double iconListWidth = GameBizIcons.Count * 52;
                x += Grid_GameIconsArea.Padding.Left + iconListWidth + Grid_GameIconsArea.Padding.Right;
            }
            double width = Math.Max(0, this.XamlRoot.Size.Width - x);
            if (Border_Pin.IsHitTestVisible && Border_Pin.Opacity > 0)
            {
                width = Math.Max(0, width - Border_Pin.ActualWidth - Border_Pin.Margin.Left - Grid_GameIconsArea.Margin.Right);
            }
            this.XamlRoot.SetWindowDragRectangles([new Rect(x, 0, width, 48)]);
        }
        catch { }
    }
    ...
    private void InitializeGameServerArea(List<GameInfo> gameInfos)
    {
        try
        {
            ...
            AddLocalGameBizDisplay(list, GameBiz.endfield);
            AddLocalGameBizDisplay(list, GameBiz.wutheringwaves);
    ...
    private void AddLocalGameBizDisplay(List<GameBizDisplay> list, GameBiz gameBiz)
    {
        if (list.Any(x => x.GameInfo.GameBiz.Game == gameBiz.Game))
        {
            return;
        }
        GameBiz displayBiz = LanguageUtil.FilterLanguage(CultureInfo.CurrentUICulture.Name) is "zh-cn" ? $"{gameBiz.Game}_cn" : $"{gameBiz.Game}_global";
        list.Add(new GameBizDisplay
        {
            GameInfo = new GameInfo
            {
                Id = GameId.FromGameBiz(displayBiz)!.Id,
                GameBiz = displayBiz,
                Display = new GameInfoDisplay
                {
                    Name = displayBiz.ToGameName(),
                    Icon = new GameImage { Url = GameBizIcon.GameBizToIcon(displayBiz) },
                    Thumbnail = new GameImage { Url = LocalGameCover(displayBiz) },
                    Background = new GameImage { Url = LocalGameCover(displayBiz) },
                    Logo = new GameImage { Url = "ms-appx:///Assets/Image/Transparent.png" },
                },
                GameServerConfigs = [],
            }
        });
    ...
    private static string LocalGameCover(GameBiz gameBiz)
    {
        return gameBiz.Game switch
        {
            GameBiz.arknights => "ms-appx:///Assets/Image/cover_arknights.jpg",
            GameBiz.endfield => "ms-appx:///Assets/Image/cover_endfield.jpg",
            GameBiz.wutheringwaves => "ms-appx:///Assets/Image/cover_wutheringwaves.jpg",
    ...
    public void AutoSearchInstalledGames()
    {
        try
        {
            List<GameInfo> gameInfos = GetCachedGameInfos();
            var sb = new StringBuilder();
            IEnumerable<GameBiz> gameBizs = gameInfos.Select(item =>
            {
                ...
                return gameBiz;
            }).Concat(GameBiz.AllGameBizs).Distinct();
            foreach (GameBiz gameBiz in gameBizs)
            {
                ...
                path = GetGameInstallPathFromRegistry(gameBiz);
    ...
    private static string? GetGameInstallPathFromRegistry(GameBiz gameBiz)
    {
        string[] keys =
        [
            gameBiz.Server is "cn" ? $@"HKEY_CURRENT_USER\Software\miHoYo\HYP\1_1\{gameBiz}" : "",
            gameBiz.Server is "global" ? $@"HKEY_CURRENT_USER\Software\Cognosphere\HYP\1_0\{gameBiz}" : "",
            gameBiz.GetGameRegistryKey(),
        ];
        string[] valueNames = ["GameInstallPath", "InstallPath", "InstallLocation"];
        foreach (string key in keys.Where(x => !string.IsNullOrWhiteSpace(x)).Distinct())
        {
            foreach (string valueName in valueNames)
            {
                string? path = Registry.GetValue(key, valueName, null) as string;
                path = path?.Trim('"');
                if (Directory.Exists(path))
                {
                    return path;
