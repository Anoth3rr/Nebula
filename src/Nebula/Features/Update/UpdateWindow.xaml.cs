...
public sealed partial class UpdateWindow : WindowEx
{
    ...
    private void Restart()
    {
        try
        {
            string? launcher = AppConfig.NebulaPortableLauncherExecutePath;
    ...
    private async Task LoadUpdateContentAsync()
    {
        try
        {
            ...
            if (!ua.Contains("Nebula"))
            {
                webview.CoreWebView2.Settings.UserAgent = $"{ua} Nebula/{AppConfig.AppVersion}";
            }
