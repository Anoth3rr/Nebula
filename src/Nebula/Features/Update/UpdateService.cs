...
internal class UpdateService
{
    ...
    private async Task StartInternalAsync(ReleaseInfoDetail release, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            string targetPath = Path.GetDirectoryName(AppConfig.NebulaPortableLauncherExecutePath)!;
    ...
    private async Task PrepareForUpdateAsync(ReleaseManifest manifest, string targetPath, CancellationToken cancellationToken = default)
    {
        try
        {
            _logger.LogInformation("Prepare for update nebula");
            State = UpdateState.Pending;
    ...
    private async Task UpdateAsync(CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            _logger.LogInformation("Update Nebula finished");
        }
        catch (OperationCanceledException)
        {
            _logger.LogInformation("Update Nebula canceled, restore version.ini");
            State = UpdateState.Stop;
            RestoreVersionIni();
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Update Nebula failed, restore version.ini");
            State = UpdateState.Error;
            ErrorMessage = ex.Message;
            RestoreVersionIni();
        }
    ...
    private async Task DownloadFilesAsync(CancellationToken cancellationToken = default)
    {
        ...
        _logger.LogInformation("Update Nebula, downloading {count} files, file size: {size}", _releaseManifest.DiffFileCount, Progress_TotalBytes);
        ...
        _logger.LogInformation("Update Nebula, downloading files finished");
    }
    ...
    {
        _logger.LogInformation("Update Nebula, patching and verifying files");
