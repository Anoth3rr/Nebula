...
public sealed partial class WelcomeWindow : WindowEx
{
    ...
    public bool CanStartNebula { get; set => SetProperty(ref field, value); }
    ...
    private void InitializeDefaultUserDataFolder()
    {
        try
        {
            ...
            {
                UserDataFolder = Path.Combine(Path.GetPathRoot(AppContext.BaseDirectory)!, ".NebulaData");
            }
            else
            {
#if DEBUG
                UserDataFolder = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "Nebula");
#else
                UserDataFolder = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments), "Nebula");
#endif
            }
        }
        catch (Exception ex)
