...
public partial class ScreenshotItem : ObservableObject
{
    ...
    {
        // nebula
        if (DateTime.TryParseExact(NebulaNameRegex().Match(name).Value, "yyyyMMdd_HHmmssff", null, DateTimeStyles.None, out time))
        {
            return true;
        }
        // genshin zzz
    ...
    private static partial Regex NebulaNameRegex();
