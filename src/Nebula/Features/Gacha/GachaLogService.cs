...
internal abstract class GachaLogService
{
    ...
    public virtual int DeleteGachaLogUrl(long uid)
    {
        using var dapper = DatabaseService.CreateConnection();
        return dapper.Execute("DELETE FROM GachaLogUrl WHERE Uid = @uid AND GameBiz = @GameBiz;", new { uid, GameBiz = CurrentGameBiz });
    }
    ...
    public virtual async Task<long> GetGachaLogAsync(string url, bool all, string? lang = null, IProgress<string>? progress = null, CancellationToken cancellationToken = default)
    {
        ...
        else
        {
            ...
            // 获取 {list.Count} 条记录，新增 {newCount - oldCount} 条记�?
