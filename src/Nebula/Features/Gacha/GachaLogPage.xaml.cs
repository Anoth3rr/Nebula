...
public sealed partial class GachaLogPage : PageBase
{
    ...
    protected override void OnNavigatedTo(NavigationEventArgs e)
    {
        ...
        IsDefaultGachaStatsCardVisible = true;
        ...
        if (CurrentGameBiz.Game == GameBiz.nap)
        {
            ...
            IsDefaultGachaStatsCardVisible = false;
        ...
        if (CurrentGameBiz.Game == GameBiz.endfield)
        {
            EnableGenericGachaItemStats = true;
            _gachaLogService = AppConfig.GetService<EndfieldGachaService>();
            Button_EndfieldLogin.Visibility = Visibility.Visible;
            MenuFlyoutItem_CloudGame.Visibility = Visibility.Collapsed;
            MenuFlyoutItem_DeleteCacheFolder.Visibility = Visibility.Collapsed;
            TextBlock_AuthkeyTimeout.Visibility = Visibility.Collapsed;
            TextBlock_DeleteCacheHint.Visibility = Visibility.Collapsed;
            StackPanel_GachaLanguage.Visibility = Visibility.Collapsed;
            Button_UIGF4.Visibility = Visibility.Collapsed;
        }
        if (CurrentGameBiz.Game == GameBiz.wutheringwaves)
        {
            EnableGenericGachaItemStats = true;
            _gachaLogService = AppConfig.GetService<WutheringWavesGachaService>();
            MenuFlyoutItem_CloudGame.Visibility = Visibility.Collapsed;
            MenuFlyoutItem_DeleteCacheFolder.Visibility = Visibility.Collapsed;
            TextBlock_AuthkeyTimeout.Visibility = Visibility.Collapsed;
            TextBlock_DeleteCacheHint.Visibility = Visibility.Collapsed;
    ...
    public bool IsDefaultGachaStatsCardVisible { get; set => SetProperty(ref field, value); }
    ...
    public bool EnableGenericGachaItemStats { get; set => SetProperty(ref field, value); }
    ...
    private async Task UpdateGachaLogAsync(string? param = null)
    {
        ...
        {
            string? url = null;
            if (CurrentGameBiz.Game == GameBiz.endfield && _gachaLogService is EndfieldGachaService endfieldGachaService)
            {
                if (SelectUid is null or 0)
                {
                    InAppToast.MainWindow?.Warning(null, "\u8bf7\u5148\u767b\u5f55\u7ec8\u672b\u5730\u8d26\u53f7\u3002");
                    return;
                }
                url = endfieldGachaService.GetGachaLogUrlByUid(SelectUid.Value);
                if (string.IsNullOrWhiteSpace(url) || !endfieldGachaService.HasSavedAccount(SelectUid.Value))
                {
                    InAppToast.MainWindow?.Warning(null, "\u5f53\u524d UID \u672a\u4fdd\u5b58\u767b\u5f55\u8d26\u53f7\uff0c\u8bf7\u5148\u70b9\u51fb\u201c\u8d26\u53f7\u767b\u5f55\u201d\u3002");
                    return;
                }
    ...
    [RelayCommand]
    private async Task LoginEndfieldAccountAsync()
    {
        try
        {
            if (_gachaLogService is not EndfieldGachaService endfieldGachaService)
            {
                return;
            }
            var dialog = new EndfieldGachaLoginDialog
            {
                XamlRoot = this.XamlRoot,
            };
            if (await dialog.ShowAsync() is not ContentDialogResult.Primary || dialog.SelectedAccount is null)
            {
                return;
            }
            endfieldGachaService.SaveAccount(dialog.SelectedAccount, dialog.LoginToken);
            Initialize();
            SelectUid = dialog.SelectedAccount.Uid;
            InAppToast.MainWindow?.Success(null, $"\u5df2\u4fdd\u5b58\u7ec8\u672b\u5730\u8d26\u53f7 {dialog.SelectedAccount.Uid}");
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Login Endfield account");
            InAppToast.MainWindow?.Error(ex);
        }
    }
    ...
    private async Task UpdateGachaLogInternalAsync(string url, bool all = false)
    {
        ...
        catch (miHoYoApiException ex)
        {
            ...
            if (CurrentGameBiz.Game is GameBiz.endfield or GameBiz.wutheringwaves)
            {
    ...
    private async Task ExportGachaLogAsync(string format)
    {
        try
        {
            ...
            var suggestName = $"Nebula_Export_{CurrentGameBiz.Game}_{uid}_{DateTime.Now:yyyyMMddHHmmss}.{ext}";
