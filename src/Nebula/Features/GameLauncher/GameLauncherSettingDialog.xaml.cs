...
public sealed partial class GameLauncherSettingDialog : ContentDialog
{
    ...
    private void InitializeGameFeature()
    {
        if (!CurrentGameBiz.IsHoYoPlayGame())
        {
            NavigationViewItem_GamePackages.Visibility = Visibility.Collapsed;
        ...
        try
        {
            if (Directory.Exists(InstallPath))
            {
                ...
                if (Directory.Exists(AppConfig.UserDataFolder))
                {
                    ...
                    {
                        // Nebula 数据文件夹位于游戏文件夹内，删除游戏时会一并删除。请在设置页面修改数据文件夹位置后重试。
                ...
                {
                    // Nebula 程序位于游戏文件夹内，删除游戏时会一并被删除。请将程序移出游戏文件夹后重试。
                    UninstallError = Lang.GameLauncherSettingDialog_UninstallGameNebulaProgramFolderWarning;
                    return;
                }
