...
public sealed partial class ScreenshotSetting : PageBase
{
    ...
    private async Task TestCaptureAsync()
    {
        try
        {
            ...
            Directory.CreateDirectory(Path.Combine(ScreenshotFolder, "Nebula"));
            ...
            filePath = Path.Join(ScreenshotFolder, "Nebula", $"Nebula_{frameTime:yyyyMMdd_HHmmssff}.{extension}");
