...
public sealed partial class AdvancedSetting : PageBase
{
    ...
    private async void CheckUrlProtocol()
    {
        try
        {
            var status = await Launcher.QueryUriSupportAsync(new Uri("nebula://"), LaunchQuerySupportType.Uri);
...
#pragma warning restore MVVMTK0034 // Direct field reference to [ObservableProperty] backing field
    ...
    private async Task TestUrlProtocolAsync()
    {
        try
        {
            await Launcher.LaunchUriAsync(new Uri("nebula://test"));
        }
        catch (Exception ex)
        {
    ...
    private async Task GetRpcServerStateAsync()
    {
        ...
        catch (RpcException ex) when (ex.Status is { StatusCode: StatusCode.DeadlineExceeded })
        {
            ...
            var process = Process.GetProcessesByName("Nebula.RPC").FirstOrDefault(x => x.SessionId == sessionId);
            if (process != null)
            {
