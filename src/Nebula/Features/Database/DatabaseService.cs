...
namespace Nebula.Features.Database;
...
internal static class DatabaseService
{
    ...
    public static void SetDatabase(string folder)
    {
        ...
        {
            _databasePath = Path.GetFullPath(Path.Combine(folder, "NebulaDatabase.db"));
...
#pragma warning disable CS0162 // 检测到无法访问的代�?
            string folder = Path.Join(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), @"Nebula\DatabaseBackup");
#pragma warning restore CS0162 // 检测到无法访问的代�?
            Directory.CreateDirectory(folder);
            string file = Path.Combine(folder, $"NebulaDatabase_AutoBackup_{DateTime.Now:yyyyMMdd_HHmmss}.db");
            string archive = Path.ChangeExtension(file, ".7z");
            string archive_tmp = archive + "_tmp";
            string[] files = Directory.GetFiles(folder, "NebulaDatabase_AutoBackup_*.7z");
            if (files.Length == 0)
            {
                BackupDatabase(file);
    ...
    private const string Sql_v20 = """
        ...
        CREATE TABLE IF NOT EXISTS WutheringWavesGachaItem
        (
            Uid       INTEGER NOT NULL,
            Id        INTEGER NOT NULL,
            Name      TEXT    NOT NULL,
        ...
        CREATE INDEX IF NOT EXISTS IX_WutheringWavesGachaItem_Id ON WutheringWavesGachaItem (Id);
        CREATE INDEX IF NOT EXISTS IX_WutheringWavesGachaItem_RankType ON WutheringWavesGachaItem (RankType);
        CREATE INDEX IF NOT EXISTS IX_WutheringWavesGachaItem_GachaType ON WutheringWavesGachaItem (GachaType);
        CREATE INDEX IF NOT EXISTS IX_WutheringWavesGachaItem_Name ON WutheringWavesGachaItem (Name);
        CREATE INDEX IF NOT EXISTS IX_WutheringWavesGachaItem_ItemId ON WutheringWavesGachaItem (ItemId);
    ...
    private const string Sql_v21 = """
        ...
        CREATE INDEX IF NOT EXISTS IX_EndfieldGachaItem_Name ON EndfieldGachaItem (Name);
        CREATE INDEX IF NOT EXISTS IX_EndfieldGachaItem_ItemId ON EndfieldGachaItem (ItemId);
    ...
    private const string Sql_v22 = """
        ...
        CREATE TABLE IF NOT EXISTS WutheringWavesGachaInfo
        (
            Id          INTEGER NOT NULL PRIMARY KEY,
            Name        TEXT    NOT NULL,
            Icon        TEXT,
