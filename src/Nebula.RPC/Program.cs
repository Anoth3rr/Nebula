...
public static class RpcRunner
{
    ...
    public static void Run(string[] args)
    {
        ...
        var logFile = Path.Combine(logFolder, $"Nebula_{DateTime.Now:yyMMdd}.log");
        Log.Logger = new LoggerConfiguration().WriteTo.File(path: logFile, shared: true, outputTemplate: $$"""[{Timestamp:HH:mm:ss.fff}] [{Level:u4}] [Nebula RPC ({{Environment.ProcessId}})] {SourceContext}{NewLine}{Message}{NewLine}{Exception}{NewLine}""")
                                          .Enrich.FromLogContext()
                                          .ReadFrom.Configuration(builder.Configuration)
                                          .CreateLogger();
        Log.Information($"Welcome to Nebula RPC v{AppConfig.AppVersion}\r\nSystem: {Environment.OSVersion}\r\nCommand Line: {Environment.CommandLine}");
        ...
        builder.Services.AddHttpClient().ConfigureHttpClientDefaults(config =>
        {
            ...
            config.ConfigureHttpClient(client =>
            {
                ...
                client.DefaultRequestHeaders.Add("User-Agent", $"Nebula.RPC/{AppConfig.AppVersion}");
            });
