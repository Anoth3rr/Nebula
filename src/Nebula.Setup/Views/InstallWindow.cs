...
using Nebula.Setup.Services;
using Nebula.Setup.Views;
...
public class InstallWindow : WindowBase
{
    ...
    private void SetDefaultInstallFolder()
    {
        try
        {
            string? folder = Registry.GetValue(@"HKLM\Software\Microsoft\Windows\CurrentVersion\Uninstall\Nebula", "InstallLocation", null) as string;
            if (!Directory.Exists(folder) || !string.Equals(new DirectoryInfo(folder).Name, "Nebula", StringComparison.OrdinalIgnoreCase))
            {
                folder = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ProgramFiles), "Nebula");
            }
            SetInstallFolder(folder);
        }
        catch
        {
            SetInstallFolder(Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ProgramFiles), "Nebula"));
        }
    }
    ...
    private async Task PrepareManifestAsync(CancellationToken cancellation = default)
    {
        try
        {
            ...
            TextBlock_TitleHeader.Text = $"{Lang.NebulaSetup}  ·  {_installService.AppVersion}";
        }
        catch (Exception ex)
        {
            Debug.WriteLine(ex);
    ...
    private async void ChangeInstallFolder()
    {
        try
        {
            ...
            {
                if (new DirectoryInfo(folder).Name is not "Nebula")
                {
                    folder = Path.Combine(folder, "Nebula");
                }
                SetInstallFolder(folder);
    ...
    private async void StartInstall()
    {
        ...
        try
        {
            if (!string.Equals(new DirectoryInfo(InstallFolder.Value).Name, "Nebula", StringComparison.OrdinalIgnoreCase))
            {
                if (SilentMode)
                {
            ...
            await File.WriteAllTextAsync(Path.Combine(InstallFolder.Value, "NebulaInstallFolder"), InstallFolder.Value);
            CreateShortcut();
    ...
    private void Launch()
    {
        try
        {
            string exe = Path.Combine(InstallFolder.Value, "Nebula.exe");
            if (File.Exists(exe))
            {
                Process.Start("explorer", $"""
                    "{Path.Combine(InstallFolder.Value, "Nebula.exe")}"
                    """);
                Environment.Exit(0);
            }
            else
    ...
    {
        CreateShortcutInternal(Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.Desktop), "Nebula.lnk"));
        string folder = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.Programs), "Nebula");
        Directory.CreateDirectory(folder);
        CreateShortcutInternal(Path.Combine(folder, "Nebula.lnk"));
    }
    ...
    private void CreateShortcutInternal(string shortcutPath)
    {
        ...
        {
            Path = Path.Combine(InstallFolder.Value, "Nebula.exe"),
