...
public class UninstallWindow : WindowBase
{
    ...
    public UninstallWindow() : base()
    {
        ...
        this.Title = Lang.NebulaUninstaller;
        BuildUI();
    }
    ...
    private async void CheckUninstall()
    {
        try
        {
            ...
            string flag = Path.Combine(currentFolder!, "NebulaInstallFolder");
            ...
            bool hasDatabase = files.Any(f => Path.GetFileName(f).Equals("NebulaDatabase.db", StringComparison.OrdinalIgnoreCase));
            if (hasDatabase)
            {
                caution = true;
    ...
    private async void Uninstall()
    {
        try
        {
            ...
            await Task.Run(() =>
            {
                ...
                {
                    string cacheFolder = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "Nebula");
    ...
    {
        string lnk = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.Desktop), "Nebula.lnk");
        if (File.Exists(lnk))
        {
            File.Delete(lnk);
        }
