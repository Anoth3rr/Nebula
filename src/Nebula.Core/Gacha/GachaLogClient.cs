...
public abstract class GachaLogClient
{
    ...
    public async Task<ZZZGachaWiki> GetZZZGachaInfoAsync(GameBiz gameBiz, string lang, CancellationToken cancellationToken = default)
    {
        ...
        if (gameBiz.IsChinaServer() && lang is "zh-cn")
        {
            const string url = $"https://nebula-static.scighost.com/metadata/v1/zzz/ZZZGachaInfo.nap_cn.zh-cn.json";
            wiki = await CommonGetAsync<ZZZGachaWiki>(url, cancellationToken);
        }
        else
        {
            string url = $"https://nebula-static.scighost.com/metadata/v1/zzz/ZZZGachaInfo.nap_global.{lang}.json";
            wiki = await CommonGetAsync<ZZZGachaWiki>(url, cancellationToken);
        }
    ...
    public async Task<List<WutheringWavesGachaInfo>> GetWutheringWavesGachaInfoAsync(CancellationToken cancellationToken = default)
    {
        var list = new List<WutheringWavesGachaInfo>();
    ...
    private async Task<List<WutheringWavesGachaInfo>> GetWutheringWavesCatalogueItemsAsync(int catalogueId, CancellationToken cancellationToken)
    {
        using var request = new HttpRequestMessage(HttpMethod.Post, "https://api.kurobbs.com/wiki/core/catalogue/item/getPage")
        {
        ...
        request.Headers.Referrer = new Uri("https://wiki.kurobbs.com/");
        request.Headers.TryAddWithoutValidation("source", "h5");
        request.Headers.TryAddWithoutValidation("wiki_type", "9");

        var wrapper = await _httpClient.SendAsync(request, cancellationToken);
        wrapper.EnsureSuccessStatusCode();
        var page = await wrapper.Content.ReadFromJsonAsync(typeof(WutheringWavesCataloguePage), GachaLogJsonContext.Default, cancellationToken) as WutheringWavesCataloguePage;
        if (page is null)
        {
            return [];
        }
        if (page.Code != 200)
        {
            throw new miHoYoApiException(page.Code, page.Message);
        }

        return page.Data?.Results?.Records
                   .Select(x => new WutheringWavesGachaInfo
                   {
                       Id = x.Id,
                       Name = x.Name ?? x.Content?.Title ?? "",
                       Icon = x.Content?.ContentUrl ?? "",
                       CatalogueId = catalogueId,
                       Rarity = x.Content?.Rarity ?? 0,
                   })
